...
- `009` was removed because its broader `role_permissions` read policy is not live in production and did not complete the below-admin access path. `010` remains for fresh replay: it adds DELETE auditing to `user_roles` and creates the `user_permissions` audit trigger. `011` and `012` are live; `012` now preserves the later deleted-actor fix if replayed. Historical `002`, `003`, and `006` abort when the private hierarchy helper exists, preventing them from restoring older function definitions on production. `013` records production security definitions applied through the sibling repository. Review each migration against live definitions before any production application.
